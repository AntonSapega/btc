...
export const PlaceOrderForm = observer(() => {
  const { activeOrderSide, price, total, amount, outputTakeProfitList, setPrice, setAmount, setTotal, setOrderSide } =
    useStore();

  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const takeProfitItems: Profit[] | null = outputTakeProfitList();
    console.log(takeProfitItems);
    if (takeProfitItems) {
      // do something
    }
  };
